...
```
shard：分片
routing：分片依据，可以是id，可以是自定义参数
number_of_primary_shards：总的分片数量-主分片
hash算法：shard = hash(routing) % number_of_primary_shards
```
创建，删除一个文档
```
必须在主分片上进行，然后主分片同步副分片成功以后才会返回成功！
当然此机制可以更改
```
取回一个文档
```
可以从主分片或者其他任性副本分片检索文档
```
